...
@pytest.mark.asyncio
async def test_get_emails(test_client_rest, dbsession):
    # Pre-populate the database with test data
    await dbsession.execute(
        sa.insert(m.EmailRecipient).values(email="test@example.com")
    )

    response = await test_client_rest.get("/email_recipients")
    data = response.json()
...
@pytest.mark.asyncio
async def test_add_email(test_client_rest, dbsession):
    response = await test_client_rest.post(
        "/email_recipients", params={"email": "new@example.com"}
    )
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_delete_email(test_client_rest, dbsession):
    ...
    response = await test_client_rest.delete(
        "/email_recipients", params={"email": "delete_me@example.com"}
    )
    assert response.status_code == 200
